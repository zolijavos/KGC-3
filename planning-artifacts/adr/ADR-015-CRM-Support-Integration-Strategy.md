...
#### 🤖 AI Chatbot (Koko) Integráció

**Döntés:** Koko AI (Google Gemini Flash) mint első szintű support ([ADR-016](ADR-016-ai-chatbot-koko.md))

**Architektúra:**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        KOKO AI ↔ CHATWOOT FLOW                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1. Ügyfél Megkeresés (Email/Chat/WhatsApp)                                │
│     └─ Chatwoot Webhook: message_created                                   │
│                                                                              │
│  2. Koko AI Screening (KGC Integration Layer)                              │
│     ├─ Intent Classification (bérlés/szerviz/értékesítés/általános)        │
│     ├─ Knowledge Base Search (pgvector semantic search)                    │
│     └─ Gemini Flash API Response Generation                                │
│                                                                              │
│  3. Confidence Decision Tree                                                │
│     ├─ ≥ 80%: Auto-response (Chatwoot API → send message)                 │
│     ├─ 50-80%: Admin Approval Queue → Chatwoot (awaiting admin)           │
│     └─ < 50%: Immediate Escalation → Human Agent (Chatwoot assignment)    │
│                                                                              │
│  4. Chatwoot Human Agent (csak ha szükséges)                               │
│     └─ Complex/sensitive ügyek, VIP partnerek                              │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| Művelet | API Végpont | Irány |
|---------|------------|-------|
| **Üzenet fogadás** | Chatwoot Webhook → KGC Integration | Chatwoot → Koko |
| **AI válasz küldés** | `POST /api/v1/conversations/{id}/messages` | Koko → Chatwoot |
| **Eszkaláció** | `PATCH /api/v1/conversations/{id}` (assign agent) | Koko → Chatwoot |
| **Admin jóváhagyás** | `POST /api/v1/conversations/{id}/messages` | Admin → Chatwoot |
| **Feedback gyűjtés** | Chatwoot Custom Attributes (👍👎) | Chatwoot → Koko KB |

**Implementációs Példa:**

```typescript
// Koko AI → Chatwoot Integration
...
app.post('/webhooks/chatwoot', async (req, res) => {
  ...
  if (kokoResponse.confidence >= 0.8) {
    ...
    await chatwootAPI.sendMessage(conversation.id, {
      content: `🤖 Koko javaslat (${Math.round(kokoResponse.confidence * 100)}% konfidencia):\n\n${kokoResponse.text}\n\n✅ Jóváhagyás szükséges`,
      message_type: 'outgoing',
    ...
    // Log escalation
    await chatwootAPI.sendMessage(conversation.id, {
      content: `🚨 Automatikusan hozzárendelve (alacsony AI konfidencia: ${Math.round(kokoResponse.confidence * 100)}%)`,
...
// Admin Approval Handler (Chatwoot UI button click → Webhook)
...
**Előnyök:**
- ✅ **40% support automatizálás**: Ismétlődő kérdések (nyitvatartás, árak, bérlési feltételek)
- ✅ **24/7 elérhetőség**: Koko válaszol éjszaka/hétvégén is
- ✅ **Gyorsabb válaszidő**: AI < 3 sec vs Human ~5-10 perc
- ✅ **Chatwoot UI változatlan**: Ügynökök ugyanazt a felületet használják
- ✅ **Human-in-the-loop**: Kritikus ügyek mindig ember kezeli
...
|-------|--------|----------|
| 2025-12-19 | 1.0 | Kezdeti dokumentum (Chatwoot + Twenty integráció) |
| 2025-12-31 | 1.1 | **Koko AI Chatbot integráció** hozzáadása (Chatwoot platform elemzés, ADR-016 referencia). 40% support automatizálás hybrid AI-human flow-val. |

---

**Készült:** BMad Orchestrator (Winston - Architect)
**Dátum:** 2025-12-19 (utolsó frissítés: 2025-12-31)
