...
export const BarcodeScanner = React.forwardRef<HTMLDivElement, BarcodeScannerProps>(
  (
    ...
    return (
      <div ref={ref} className={cn('flex flex-col gap-4', className)} data-testid="barcode-scanner">
        {/* Header */}
        ...
        {(keyboardLastScan || lastScanSource === 'camera') && (
          ...
          >
            ...
            <p className="font-mono font-medium">{keyboardLastScan?.value}</p>
          </div>
        )}
        ...
        {showCameraFallback && (
          <div className="pt-4 border-t">
            <p className="text-sm text-muted-foreground mb-3">Vagy használja a kamerát:</p>
            <CameraScanner
              onScan={result => handleScan(result, 'camera')}
              {...(onError ? { onError } : {})}
              {...(cameraConfig ? { config: cameraConfig } : {})}
              autoStart={autoStartCamera}
              {...(cameraClassName ? { className: cameraClassName } : {})}
            />
          </div>
        )}
      </div>
